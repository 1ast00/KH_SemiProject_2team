...
<head>
    ...
    <script>
        // DOMContentLoaded 이벤트 리스너를 추가하여 페이지 로드 후 스크립트 실행
        document.addEventListener('DOMContentLoaded', function() {
            // MissingSearchController에서 설정한 noResults 속성 값을 확인
            <c:if test="${requestScope.noResults == true}">
                alert("해당 실종자 데이터가 존재하지 않습니다.");
            </c:if>
        });
    </script>
</head>
    ...
    <main>
                    ...
                    <div class="person-card">
                        <div class="card-image">
                            <c:choose>
                                ...
                                <c:otherwise>
                                    <img src="${pageContext.request.contextPath}/resource/img/default.jpg" alt="기본 이미지">
                                </c:otherwise>
                            </c:choose>
                        </div>
        ...
        <div class="pagination">
            ...
            <c:if test="${paging.prev}">
                <a href="missingSearch.do?page=${paging.startPage - 1}&name=${param.name}&age=${param.age}&gender=${param.gender}&place=${param.place}&etc=${param.etc}">&laquo; 이전</a>
            </c:if>
            ...
            <c:forEach var="num" begin="${paging.startPage}" end="${paging.endPage}">
                <c:choose>
                    ...
                    <c:otherwise>
                        <a href="missingSearch.do?page=${num}&name=${param.name}&age=${param.age}&gender=${param.gender}&place=${param.place}&etc=${param.etc}">${num}</a>
                    </c:otherwise>
                </c:choose>
            </c:forEach>
            ...
            <c:if test="${paging.next}">
                <a href="missingSearch.do?page=${paging.endPage + 1}&name=${param.name}&age=${param.age}&gender=${param.gender}&place=${param.place}&etc=${param.etc}">다음 &raquo;</a>
            </c:if>
        </div>
    </main>
</div>
