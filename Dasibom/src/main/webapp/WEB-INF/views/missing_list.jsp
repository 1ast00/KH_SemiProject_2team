          ...
          <td>
            <c:choose>
              <c:when test="${not empty person.image}">
                <!-- 이미지 경로에 contextPath와 uploads 폴더 경로를 추가 -->
                <img src="${pageContext.request.contextPath}/uploads/${person.image}" width="100" height="100" alt="${person.name} 이미지" />
              </c:when>
              <c:otherwise>
                이미지 없음
              </c:otherwise>
